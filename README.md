...
历史日线现已支持多源切换，可在 `auto / eastmoney / sina / netease / sohu / ths / wscn / baostock / tdx / finshare` 之间切换。
...
- 历史数据源支持 `auto / eastmoney / sina / netease / sohu / ths / wscn / baostock / tdx / finshare`；`auto` 会优先东财，失败后切换到其它带成交额的完整历史源；`finshare` 是试验源，仅在显式选择时使用
...
也可以不打开 GUI，直接通过命令执行日常任务：

```bash
# 更新历史缓存
python main.py update-cache --max-stocks 0 --workers 3 --source auto

# 预测当天数据；非交易日会自动回退到最近交易日
...
每次预测完成后（GUI 或 `predict_today.bat`），程序会把最新一次预测（候选名单、预测依据、
情绪上下文、派生模拟买入）写到 `snapshots/latest_prediction.json`，并自动
`git commit + push`（只提交这一个文件；推送失败只在日志告警，不影响预测）。
...
GUI 启动时会自动检测快照比本地数据库新则导入，之后可直接点「竞价确认」。
说明：

- 只同步最新一份预测（覆盖式），历史回放/回测结果不会导出
- 快照不含概念分析明细（约 3MB），另一台电脑打开时会自动本机补齐题材数据
- 判定新旧用快照的 `exported_at` 对比本地记录的 `saved_at`，两台电脑时钟差按分钟级容忍
...
- 当前的“涨停原因”优先来自复盘页的题材/事件归因；东方财富强势股池的“入选理由”降级为“强势标签”
